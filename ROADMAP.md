...
**Theme**: Reach + Reusability.

- **First-time pairing over USB**, then mDNS auto-reconnect via `_adb-tls-connect._tcp` discovery on subsequent runs. PhoneFork stores per-device key fingerprints, refuses unknown bindings.
- **Pairing-code QR scanner** — drive `adb pair <ip>:<port> <code>` from a QR generated by the source phone's Wireless-Debugging settings panel.
- **Trusted-WLAN auto-start** for the companion APK (Shizuku v13.6.0 pattern). Eliminates re-pair-per-reboot.
- **USB-only enforcement default** — `phonefork --allow-wireless` opt-in flag. Source: [arXiv 2401.08961](https://arxiv.org/abs/2401.08961) ADB-security paper.
- **Per-install ADB RSA key** in `%LOCALAPPDATA%\PhoneFork\adb-keys\` (not the shared `%USERPROFILE%\.android\adbkey`) — same paper.
