...
- The window now reopens at the size, position, maximised state and tab it was last closed at, stored in `%LOCALAPPDATA%\PhoneFork\window-state.json`. A saved position is only restored when it still lands on a connected display, so unplugging a monitor between runs cannot open PhoneFork off-screen.
- The test project moves from the deprecated xunit 2.9.3 to xunit.v3 4.0.0, which runs on Microsoft.Testing.Platform. `global.json` selects MTP mode, so `dotnet test` no longer accepts VSTest-era flags such as `--nologo` and takes `--project` rather than a bare project path. All 294 tests pass unchanged.
- Screen-reader support across the app. Every DataGrid, tab, text box and dropdown now carries an accessibility name, and the generic per-tab action buttons say which domain they act on rather than just `Scan`. A test parses the shipped XAML and fails the build when a new control is added without one.
- Debloat now warns before disabling a package that something still enabled depends on. Upstream records a `neededBy` list per package, so `debloat apply` and the Debloat tab both name the survivors, and the CLI records them in the receipt. This is the failure behind reports like SIM Manager crashing after a debloat: `com.samsung.oda.service` is needed by `com.samsung.android.app.telephonyui`.
- On Android 17 the helper's SMS export now carries a warning that one-time-password messages newer than three hours may be missing. Android 17 withholds those from any app that is not the intended recipient, and the helper is never the default SMS app, so the rows were being dropped with no error at all. The host surfaces the warning in the export output and the receipt.
- The helper's restore path no longer pretends to have a confirmation gate. `rejectRestoreWithoutConfirmation` inspected a `confirmRestore` value and then refused on both branches; it is now an unconditional refusal matching what the query side already reported.
- The backup format is no longer described as App Manager compatible, because it never was. App Manager 4.1.1 writes an encrypted `meta_v5.am.json` beside a plaintext `info_v5.am.json` with different field names, so neither tool could read the other's backups. The metadata file is renamed from `meta.am.v5` to `phonefork-backup.v1.json`, and `backup export-appmanager` / `install-appmanager` become `backup export` / `backup install`. Backups written by an earlier build still load.
- Saved Wi-Fi keys are now exportable. `phonefork wifi list --with-psk` reads them through the agent, which calls `getPrivilegedConfiguredNetworks()` as the shell user (Android 11+), so no root and no Shizuku install is needed. Keys are masked in the table unless `--show-psk` is passed, and never reach the audit log or a receipt. When the privileged read is unavailable the command says why and falls back to SSID-only enumeration.
